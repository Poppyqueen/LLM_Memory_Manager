...
class memory_manager:
    ...
    def __init__(self, ai_bot_instance=None):
        ...
        """
        self.filepath = "memory_database.json"
...
if __name__ == "__main__":
    ...
    for i in range(15):
        memory_system.save_to_memory(f"Short-term message {i+1} from User.", "short_term", "user")

    # Fill up long-term memory to trigger check
    for i in range(8):
        memory_system.save_to_memory(f"Long-term memory entry {i+1}.", "long_term")

    memory_system.save_to_memory("Initial base memory content.", "base_memory")

    print("\n--- Memory Structure Before Check (Raw) ---")
    print(memory_system.get_memory_structure()) # Raw dictionary

    print("\n--- Memory Structure Before Check (Formatted) ---")
    print(memory_system.get_memory_structure(formatted=True)) # Formatted JSON string

    memory_system.check()  # Run the memory check and summarization
    ...
    for i in range(12):
        memory_system.save_to_memory(f"Additional short-term message {i+1}.", "short_term", "user")
